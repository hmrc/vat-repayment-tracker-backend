...
@Singleton
class Controller @Inject() (cc: ControllerComponents, vrtRepo: VrtRepo)(implicit executionContext: ExecutionContext) extends BackendController(cc) {

  def storeRepaymentData(): Action[VrtRepaymentDetailData] = Action.async(parse.json[VrtRepaymentDetailData]) { implicit request =>
  ...
  def findRepaymentData(vrn: Vrn, periodKey: PeriodKey): Action[AnyContent] = Action.async { implicit request =>
